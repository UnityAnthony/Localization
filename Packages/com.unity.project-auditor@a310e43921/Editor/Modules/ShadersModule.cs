...
*/
       ...
       // protected ViewDescriptor m_Desc;
        internal static void GenerateShaderVariantCollection()
        {
           // Debug.Log("GenerateShaderVariantCollection start");
            //  static string s_ExportDirectory = string.Empty; string.Format("project-auditor-{0}.csv", m_Desc.category.ToString()).ToLower()
            var path = EditorUtility.SaveFilePanel("Save to SVC file", s_ExportDirectory, "NewShaderVariants.shadervariants",
                "shadervariants");
            if (path.Length != 0)
            {
                Debug.Log("GenerateShaderVariantCollection made"  + path);
